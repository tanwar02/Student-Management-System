...
const Student = () => {
  ...
  return (
    ...
    <div class="card text-center container" style={{marginTop: 100}}>
      ...
      <h5 class="card-title fs-1 fw-bold my-4">{location.state?.res.name}</h5>
      <div class="card-body">
        <div className="card-data flex-column fw-bold ms-3 me-5">
          <p class="card-text fs-4 d-flex">ID : {location.state?.res.id}</p>
          <p class="card-text fs-4 d-flex">Branch : {location.state?.res.branch}</p>
          <p class="card-text fs-4 d-flex">Course : {location.state?.res.course}</p>
